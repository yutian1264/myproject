(function ($) {
  var tempDirectory=""
  $.fn.easyUpload = function (opts,dir) {
    var path="H:/"
    var defaults = {
      dir:"upload",
      ...
      var easyManager = {
        ...
        bindHead: function () {
          ...
          $('.easy_upload_head_btn1').off('click').click(function(){
            ...
            if (arr.length>0) {
              allowFiles =arr
              //为什么要合并呢。单一图片连续上传会造成多个
              // allowFiles = allowFiles.concat(arr);
              if(StringsUtils.isNull(tempDirectory)){
                alert("请选择目录！")
                return
              }
              tempDirectory=path+tempDirectory+"/"
              if(option.beginFunc!=null){
                option.beginFunc(allowFiles.length);
              }
              postedNum=0
              upFiniehed = true;
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += '</div>';
            // $html+='<div><span>描述:</span><input id="desc"></div>'
          ...
          for (var i = 0; i < fileArr.length; i++) {
            if (option.multi) {
              var qItemNum = $(queueUl).find('.easy_upload_queue_item').length;
          ...
          function upload() {
            if (allowNewPost) {
              ...
              if (params!=null && params.length) {
                  for(key in params[postedNum])
                  fd.append(key, params[postedNum][key]);

              }
              fd.append("savePath",tempDirectory)
              ...
              $.ajax({
                ...
                success: function (result) {
                  ...
                  if (res.code!=option.okCode|| res.status!=1){
                    allowNewPost = true;
                    if (option.multi) {
        ...
        _findItems: function(type,target) {
          var arr = [];
          var icon=$(target).find("li")
          for (var i = 0; i < icon.length; i++) {
            arr.push(i);
            params.push({name:$(icon[i]).find("#fname").val()})
            //,desc:$(icon[i]).find("#desc").val()
  ...
  // 定义暴露format函数
  $.fn.easyUpload.setDir = function(txt) {
    tempDirectory=txt
  };
}(jQuery));
